...
#     query_boxes: (K, 4) ndarray of flo
#     Returns
#    -------
...
class VoxelPostprocessor(BasePostprocessor):
    ...
    #
    #     #bus anchor
    #     bus_l = self.params['anchor_args']['bus_l']
    #     bus_w = self.params['anchor_args']['bus_w']
    #     bus_h = self.params['anchor_args']['bus_h']
    #     bus_r = self.params['anchor_args']['bus_r']
    #     bus_r = [math.radians(ele) for ele in bus_r]
    #
    ...
    #
    #     self.Car_anchor_num = self.anchor_num//2
    #
    #     cx, cy = np.meshgrid(x, y)
    #     cx = np.tile(cx[..., np.newaxis], self.Car_anchor_num)
    #     cy = np.tile(cy[..., np.newaxis], self.Car_anchor_num)
    ...
    #
    #     #bus_anchor
    #     bus_w = np.ones_like(cx) * bus_w
    #     bus_l = np.ones_like(cx) * bus_l
    #     bus_h = np.ones_like(cx) * bus_h
    #
    #
    #     r_ = np.ones_like(cx)
    #     for i in range(self.Car_anchor_num):
    #         r_[..., i] = r[i]
    #
    #
    #
    #     if self.params['order'] == 'hwl':
    #         anchors_car = np.stack([cx, cy, cz, h, w, l, r_], axis=-1)
    #         anchors_bus = np.stack([cx, cy, cz, bus_h, bus_w, bus_l, r_], axis=-1)
    #         anchors = np.concatenate([anchors_car, anchors_bus],axis=2 )
    #
    ...
    def collate_batch(label_batch_list):
        ...
        return {'targets': targets,
                ...
                'neg_equal_one': neg_equal_one}
    """
    written by He caizhen 20240603
    """
    def post_process_single(self, data_dict, output_dict):
        """
        ...
        for cav_id, cav_content in data_dict.items():
            ...
            if len(boxes3d) != 0:
                ...
                pred_box3d_list.append(boxes3d_corner)
    ...
    @staticmethod
    def collate_batch_bev(holistic_bev_list):
        holistic_bev_dict = {}

        for i in range(len(holistic_bev_list)):
            batch_bev = []
            for j in range(len(holistic_bev_list[i])):
                batch_bev.append(holistic_bev_list[i][j])
            batch_bev = torch.from_numpy(np.array(batch_bev))
            holistic_bev_dict[i] = batch_bev

        return holistic_bev_dict
