...
class DistillationLoss(nn.Module):
    ...
    def forward(self, teacher_model,student_model):
        #计算蒸馏后的结果以及soft_loss
        teacher_model = teacher_model.view(teacher_model.size(0), -1)
        student_model = student_model.view(student_model.size(0), -1)
        distillation_loss = torch.abs(self.soft_loss(
            F.softmax(teacher_model / self.temp, dim = 1),
            F.softmax(student_model / self.temp, dim = 1 )
        ))
...
class combineLoss(nn.Module):
    ...
    def forward(self, teacher_model,student_model, output_dict, target_dict):
        """
        Parameters
        ----------
        output_dict : dict
        ...
        cls_preds = psm.permute(0, 2, 3, 1).contiguous()#预测分类
        ...
        #distillation loss
        distillation_loss =\
            self.distillation(teacher_model,student_model)


        reg_loss = loc_loss_src.sum() / rm.shape[0]
        reg_loss *= self.reg_coe
        total_loss = reg_loss + conf_loss
        distillation_combine_loss = self.distillation_alpha*(reg_loss + conf_loss)+ \
        ...
        return distillation_combine_loss
        # if self.train:
        #     return distillation_combine_loss
        # else:
        #     return total_loss
    ...
    def logging(self, epoch, batch_id, batch_len, writer, pbar=None):
        ...
        if pbar is None:
            print("[epoch %d][%d/%d], || KL_Loss: %.4f || Loss: %.4f "
                " || conf Loss: %.4f" % (
                    epoch, batch_id + 1, batch_len,KL_loss.item(),
                    total_loss.item(), conf_loss.item()))
        else:
            pbar.set_description("[epoch %d][%d/%d], || KL_Loss: %.4f || Loss: %.4f "
                  " || conf Loss: %.4f" % (
                      epoch, batch_id + 1, batch_len,KL_loss.item(),
                      total_loss.item(), conf_loss.item()))
