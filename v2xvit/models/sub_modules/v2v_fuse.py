...
class V2VNetFusion(nn.Module):
    ...
    def forward(self, x, record_len, pairwise_t_matrix, prior_encoding):
        ...
        # iteratively update the features for num_iteration times根据次数进行迭代
        for l in range(self.num_iteration):

            batch_updated_node_features = []
            # iterate each batch 迭代每个Batch
            for b in range(B):
                # number of valid agent 有效人数
                N = record_len[b]
                # (N,N,4,4)
...
class V2VNetFusion(nn.Module):
    def __init__(self, args):
        ...
        self.cnn = nn.Sequential(
            nn.Conv2d(in_channels + 1, in_channels, kernel_size=3,
                      stride=1, padding=1),
        ...
        for l in range(self.num_iteration):
            ...
            for b in range(B):
                ...
                for i in range(N):
                    ...
                    if self.gru_flag:
                        gru_out = \
                            self.conv_gru(cat_feature.
                                          unsqueeze(0).unsqueeze(0))[0][0].squeeze(0).squeeze(0)
                    else:
                        gru_out = batch_node_features[b][i, ...] + agg_feature
                    updated_node_features.append(gru_out.unsqueeze(0))
                # (N,C,H,W)
