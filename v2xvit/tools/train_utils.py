...
def load_saved_model(saved_path, model):
    ...
    if initial_epoch > 0:
        ...
        combine_dict= torch.load(
            os.path.join(saved_path,'net_epoch%d.pth' % initial_epoch))
        if len(combine_dict) == 2:
            # distillation edition
            model.load_state_dict(combine_dict[0], strict=False)
        else:
            model.load_state_dict(combine_dict, strict=False)
...
def create_model(hypes):
    ...
    for name, cls in model_lib.__dict__.items():
        if name.lower() == target_model_name.lower():#.lower()函数转化为小写
            model = cls
...
def create_teacher_model(hypes):
    """
    Import the module "models/[model_name].py
    ...
    model : opencood,object
        Model object.
    """
    backbone_name = hypes['teacher_model']['core_method']
    backbone_config = hypes['teacher_model']['args']
...
def create_combine_loss(hypes):
    ...
    criterion : opencood.object
        The loss function.
    """
    loss_func_name = hypes['combine_loss']['core_method']
    loss_func_config = hypes['combine_loss']['args']
...
def setup_lr_schedular(hypes, optimizer):
    ...
    if lr_schedule_config['core_method'] == 'step':
        ...
        scheduler = MultiStepLR(optimizer,
                                ...
                                gamma=gamma)
    #余弦退火学习率
    elif lr_schedule_config['core_method'] == 'CosineAnnealingWarmRestarts':
        from torch.optim.lr_scheduler import CosineAnnealingWarmRestarts
        T_0 = lr_schedule_config['T_0']
        eta_min = lr_schedule_config['eta_min']
        scheduler = CosineAnnealingWarmRestarts(optimizer,
                                T_0=T_0,
                                eta_min=eta_min)
    elif lr_schedule_config['core_method'] == 'CosineLRScheduler':
        from torch.optim.lr_scheduler import CosineLRScheduler
        t_initial = lr_schedule_config['t_initial']
        lr_min = lr_schedule_config['lr_min']
        warmup_lr_init = lr_schedule_config['warmup_lr_init ']
...
def combineTrain(model_student, model_teacher,batch_data):


    student_dict, output_dict = model_student(batch_data[0]['ego'])
    teacher_dict = model_teacher(batch_data[1]['ego'])
    return student_dict,teacher_dict,output_dict
