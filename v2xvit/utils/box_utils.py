...
from v2xvit.utils.transformation_utils import x1_to_x2,x_to_world
import copy
...
def get_mask_for_boxes_within_range_torch(boxes):
    ...
    device = boxes.device
    # print(torch.Tensor(GT_RANGE[:2]).reshape(1, 1, -1))
    # print(torch.Tensor(GT_RANGE[3:5]))
    ...
    # boundary_lower_range = torch.Tensor([[[-140.000,-38.4000]]]).to(device)
    # boundary_higher_range = torch.Tensor([[[140.000,38.4000]]]).to(device)
    mask = torch.all(
...
def load_single_objects_dairv2x(object_list,
    ...
    for object_content in object_list:
        ...
        bbx_lidar = [x, y, z, h, w, l, rotation] if order == "hwl" else [x, y, z, l, w, h,
                                                                         rotation]  # suppose order is in ['hwl', 'lwh']
