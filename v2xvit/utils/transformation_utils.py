...
# Note:
# 这里的Yaw,Pitch,Roll为CARLA坐标系统，为左手坐标系，但与常规定义的左手坐标系的Yaw,Pitch,Roll 有所不同
# CARLA坐标系统下：Yaw为Z轴正半轴往原点看顺时针旋转为正， Pitch为Y轴正半轴往原点看逆时针旋转为正，Roll为X轴正半轴往原点看逆时针旋转为正
# 常规的左手坐标系，均应该是正半轴往原点看顺时针旋转为正。

# In addition，常规的右手坐标系，均应该是正半轴往原点看逆时针旋转为正

# 这导致了CARLA的（Yaw，Pitch，Roll）做transformation matrix做转化的时候，与常规的公式（http://msl.cs.uiuc.edu/planning/node102.html）在pitch和roll上都有正负号的差异，体现在他们的sin和cos上
# 由于变换矩阵是consist的，在本套代码里就follow CARLA坐标系统的设定。转化回yaw，pitch，roll的时候遵循CARLA这一套
# V2X-Sim和DAIRV2X，都是先获得变换矩阵，再通过这一套转换转为[x,y,z,roll,yaw,pitch]的位姿描述
...
from v2xvit.utils.common_utils import check_numpy_to_torch
