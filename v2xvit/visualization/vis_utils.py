...
def linset_assign_list(vis,
    ...
    for j in range(len(lineset_list1)):
        ...
        # print(index)
        if index >= len(lineset_list2):
            print("index:",str(index))
            print('len:',str(len(lineset_list2)))
...
def visualize_single_sample_output_gt(pred_tensor,
    ...
    def custom_draw_geometry(pcd, pred, gt,save_path):
        vis = o3d.visualization.Visualizer()
        vis.create_window(width=3200,height=2400)
        ...
        pcd.paint_uniform_color([0.5,0.5,0.5]) #设置点云颜色为红色
        # for ele in pred:
        #     vis.add_geometry(ele)
        # for ele in gt:
        #     vis.add_geometry(ele)

        view_control.set_zoom(0.22)
        vis.poll_events()
        vis.update_renderer()
    ...
    if show_vis:
        custom_draw_geometry(o3d_pcd, oabbs_pred, oabbs_gt,save_path)
...
def visualize_sequence_dataloader(dataloader, order, color_mode='constant'):
    ...
    #设置o3d的一些参数
    vis = o3d.visualization.Visualizer()
    vis.create_window(width=3200, height=3200)
    ...
    # used to visualize object bounding box, maximum 50 显示物品边界框的数量
    vis_aabbs = []
    for _ in range(50):
